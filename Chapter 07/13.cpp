...
void calculateAverage(double[][TESTS], vector<double>&, string[]);
...
int main() {
	...
	calculateAverage(test_results, average_scores, student_names);
...
void calculateAverage(double array[][TESTS], vector<double>& scores, string names[]) {
	...
	for (int i = 0; i < STUDENTS; i++) {
		...
		total -= lowest;
		cout << names[i] << "'s score of " << lowest << " was dropped." << endl;
		average = total / static_cast<double>(TESTS) - 1;
