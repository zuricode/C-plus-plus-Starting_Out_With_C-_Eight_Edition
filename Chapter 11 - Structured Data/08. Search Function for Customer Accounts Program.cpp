...
//name as an argumentand then search for an account with a name that matches it.All
...
void searchForCustomer(Customer[], const int, bool);
...
int main() {
	...
	do {
		...
		case 3:
			...
			break;
		case 4:
			searchForCustomer(list, AMOUNT, isData);
			break;
		case 5:
			quit = true;
			break;
...
int Menu() {
	...
	cout << "4. Customer search function" << endl;
...
void searchForCustomer(Customer c[], const int AMOUNT, bool isData) {
	...
	if (isData) {
		...
		cout << "-------------------------------------------" << endl;
		cout << "*******  CUSTOMER SEARCH FUNCTION  ********" << endl;
		cout << "-------------------------------------------" << endl;
		cout << endl;

		cout << "Enter part (or all) of the customer's name: ";
		getline(cin, text);
		...
		for (int i = 0; i < AMOUNT; i++) {
			...
			if (c[i].name.find(text, 0) < c[i].name.length()) {
				isMatch = true;
				matches.push_back(i);
			}
		...
		if (isMatch) {
			...
			cout << "Matches found for \"" << original_text << "\": " << endl;
			cout << endl;

			for (int i = 0; i < matches.size(); i++) {
				num = matches[i];
				cout << "#" << matches[i] + 1 << ". " << c[num].name << endl;
			}

		}

		else {
			cout << "No matches were found in the database for \"" << original_text << "\"." << endl;
		}

	}

	else {
		cout << "ERROR: There is no data in the system to search!" << endl;
	}
